...
isdir = file.info(config$path)$isdir

if (!is.na(isdir) & isdir == TRUE) {
	setwd(config$path)
	...
	Sys.setFileTime("../raw-data/fileloc.rds", Sys.time())
	print("Git repo doesn't exist. Aborting execution.")
}
